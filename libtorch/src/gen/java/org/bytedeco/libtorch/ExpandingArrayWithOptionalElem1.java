...
/** A utility class that accepts either a container of {@code D}-many {@code c10::optional<T>} values,
 *  or a single {@code c10::optional<T>} value, which is internally repeated {@code D} times.
 *  It has the additional ability to accept containers of the underlying type {@code T} and
 *  convert them to a container of {@code c10::optional<T>}. */
@Name("torch::ExpandingArrayWithOptionalElem<1>") @Properties(inherit = org.bytedeco.libtorch.presets.libtorch.class)
public class ExpandingArrayWithOptionalElem1 extends Pointer {
    static { Loader.load(); }
   ...
   *  The extent of the length is checked against the {@code ExpandingArrayWithOptionalElem}'s extent parameter {@code D}
   *  at runtime. */
  /*implicit*/ public ExpandingArrayWithOptionalElem1(@StdVector T vec) { super((Pointer)null); allocate(vec); }
private native void allocate(@StdVector T vec);
   ...
   *  The extent of the length is checked against the {@code ExpandingArrayWithOptionalElem}'s extent parameter {@code D}
   *  at runtime. */
  /*implicit*/ public ExpandingArrayWithOptionalElem1(@ByVal @Cast("at::ArrayRef<T>*") Pointer values) { super((Pointer)null); allocate(values); }
