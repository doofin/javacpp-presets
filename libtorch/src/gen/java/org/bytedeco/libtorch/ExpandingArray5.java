...
@Name("torch::ExpandingArray<5>") @NoOffset @Properties(inherit = org.bytedeco.libtorch.presets.libtorch.class)
public class ExpandingArray5 extends Pointer {
    static { Loader.load(); }
   ...
   *  the length is checked against the {@code ExpandingArray}'s extent parameter {@code D}
   *  at runtime. */
  /*implicit*/ public ExpandingArray5(@StdVector T vec) { super((Pointer)null); allocate(vec); }
private native void allocate(@StdVector T vec);

  /** Constructs an {@code ExpandingArray} from an {@code at::ArrayRef}. The extent of
   *  the length is checked against the {@code ExpandingArray}'s extent parameter {@code D}
   *  at runtime. */
  /*implicit*/ public ExpandingArray5(@ByVal @Cast("at::ArrayRef<T>*") Pointer values) { super((Pointer)null); allocate(values); }
private native void allocate(@ByVal @Cast("at::ArrayRef<T>*") Pointer values);
  ...
  public native @Cast("std::array<T,5>*") @ByRef @Name("operator *") Pointer multiply();

  /** Accesses the underlying {@code std::array}. */

  /** Accesses the underlying {@code std::array}. */
  public native @Cast("std::array<T,5>*") @Name("operator ->") Pointer access();
